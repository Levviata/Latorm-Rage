...
{
	public class PolishedLatormTexture : ModTile	//TileExample1 its the item name, change with your's
	{
		...
		public override void SetDefaults()
		{
			...
			dustType = mod.DustType("Platinium");
			drop = mod.ItemType("PolishedLatorm"); //change "LatormOre" to your's ore name, if you're confused dont put "TileExample1" in here, ("TileExample1" is the texture that will appear when you create the world) put "LatormOre" (I suppose that you're asking "why i need to put "OreName" and not "TileExample1" Because "OreName" its the item that you gonna obtain, "TileExample1" again its the texture that appear when you creates a world)
			AddMapEntry(new Color(200, 200, 200));
			minPick = 20; //the minimum pickaxe power to pick the ore/block.
            mineResist = 1f; //you last more picking the ore/block.	(for these that are new f = float, when you reach 10 you dont need to quit the f you just do "item.blabla = 10f;" "item.blabla = 100f;", you never quit the f) 
		}
